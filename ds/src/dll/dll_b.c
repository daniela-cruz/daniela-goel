...
dll_iter_t DLLInsert(dll_iter_t iterator, const void *data)
{
	...
	iterator.prev->npx = NodeXOR(DLLIterPrev(iterator).prev, new_node);
	iterator.curr_node->npx = NodeXOR(DLLIterNext(iterator).curr_node, new_node);
...
{
	(1 == DLLIterIsEqual(to, DLLEnd(to.list))) ? to = DLLIterPrev(to) : to;
